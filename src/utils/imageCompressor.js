function compressWithCanvas(img, originalFile, { maxWidth, quality, maxSizeMB }) {
  return new Promise((resolve) => {
    ...
    canvas.toBlob(async (blob) => {
      ...
      if (blob.size > maxSizeBytes && quality > 0.3) {
        const nextQuality = Math.round((quality - 0.1) * 10) / 10;
        canvas.toBlob((nextBlob) => {
          ...
          const reduction = ((originalFile.size - nextBlob.size) / originalFile.size) * 100;
          resolve({
            file: compressedFile,
            originalSize: originalFile.size,
            compressedSize: nextBlob.size,
